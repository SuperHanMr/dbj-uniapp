<template>
  <view class="decorate">
    <view class="waterfall-box h-flex-x h-flex-2">
      <view>
        <waterfall v-for="(item,index) in leftList" :key="index" :params="item" tag="left" :index="index"
          @height="onHeight" @click="onClick(index, '0')" @collection="onCollection(index, '0')"></waterfall>
      </view>
      <view>
        <waterfall v-for="(item,index) in rightList" :key="index" :params="item" @height="onHeight"
          @click="onClick(index, '1')" @collection="onCollection(index, '1')" tag="right" :index="index"></waterfall>
      </view>
  ...
  export default {
    ...
    methods: {
      ...
      onCollection(index, tag) {
        this.$emit("onCollection", index, tag);
