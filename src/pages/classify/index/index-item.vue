  ...
  <view class="page-body">
    ...
    >
      <view class="left-title-block">
        ...
        <view
          class="nav-left-item"
          :class="{'nextNode':  categoryActive === detailData.length - 1}"
        ></view>
      </view>
    </scroll-view>
    <scroll-view
    ...
    >
      <view
        class="right-view"
        v-for="(menu3, index3) in detailData[categoryActive]['children']"
        :key="index3"
      >
        <view v-if="menu3['children'].length">
          ...
          >
            <view class="card-box">
              <view
                class="right-detail"
                v-for="(detail, detailK) in menu3['children']"
                :key="detailK"
...
export default {
  ...
  data() {
    return {
      ...
      scrollLeftTop: 0,
      arr: [
        0, 584, 1168, 1752, 2336, 2805, 3274, 3858, 4442, 4911, 5380, 5734,
        6203, 6672, 7017,
      ], //初始值，后边计算会根据手机适配覆盖
    ...
    categoryClickMain(index) {
      ...
      this.diff > 0 &&
        (this.scrollLeftTop = Math.round(
          (this.categoryActive * this.diff) / (this.detailData.length - 1)
        ));
    },
    toGoodsList(name, id) {
      uni.navigateTo({
