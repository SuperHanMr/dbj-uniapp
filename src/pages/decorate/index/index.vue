<template>
  <view v-if="isLogin">
    <view
      class="decorate-index"
      v-if="estateList.length > 0"
    >
      <image
        class="bg-index"
        mode="aspectFit"
      ...
      <view class="content flex-column">
        <view class="house-firend">
            ...
            >
              ...
              <view class="friend-list">
                <image
                  ...
                  :class="{avtor1: index == 0, avtor1: index == 0,avtor2: index == 1}"
                ></image>
          ...
          <view class="uni-padding-wrap">
            <view class="insurance-house">
              <view
                :class="{'payed':aServiceData.insuranceStatus}"
                class="insurance"
              >
                <image
                  @click="consultingService"
            ...
            <view class="picture-btn-wrap">
              <picture-btn
                v-if="aServiceData.showDesignFlag"
                iconUrl="https://ali-image.dabanjia.com/static/mp/dabanjia/images/decorate/ic_sgtz.svg"
                class="p-i-t"
                text="施工图纸"
                @gotoPage="goDesignPicture"
              >
              </picture-btn>
              <picture-btn
                v-if="aServiceData.showActuaryFlag"
                iconUrl="https://ali-image.dabanjia.com/static/mp/dabanjia/images/decorate/ic_jsd.svg"
                class="p-i-t"
                text="精算单"
                @gotoPage="goActuary"
              >
              ...
              <picture-btn
                v-if="aServiceData.showVideoFlag"
                iconUrl="https://ali-image.dabanjia.com/static/mp/dabanjia/images/decorate/ic_gdsp.svg"
                class="p-i-t"
                text="工地视频"
                @gotoPage="goVideo"
              ></picture-btn>
              <picture-btn
                v-if="aServiceData.constructionFlag"
                iconUrl="https://ali-image.dabanjia.com/static/mp/dabanjia/images/decorate/ic_sg.svg"
                text="施工"
                @gotoPage="goConstrction"
              ></picture-btn>
            </view>
          </view>
        </view>
        ...
        <view class="scroll-view flex-1">
          ...
          <text-scroll
            :show="showScroll"
            :dataList="broadcastList"
            v-if="isConstruction"
            @goDecorateCalendar="goDecorateCalendar"
          ></text-scroll>
          <!-- 我的仓库 -->
          <view
            v-if="haveWarehouse"
            class="my-decorate-service-wrap"
          >
            <view class="my-decorate-service ck">
              <view class="service-title flex-space-between-row">
                <text class="t">{{who}}的仓库</text>
                <view
                  class="r flex-start-row"
                  @click="goToMyWarehouse"
                >
                  <text>查看全部</text>
              ...
              <view class="my-warehouse">
                ...
                <mwarehouse-btn
                  iconUrl="https://ali-image.dabanjia.com/static/mp/dabanjia/images/decorate/ic_ck_tk.svg"
                  @gotoPage="gotoPage('3')"
                  name="退款"
                >
                </mwarehouse-btn>
          ...
          <!-- 我的装修服务 -->
          <view
            class="my-decorate-service-wrap my-decorate-service-dec-wrap"
          ...
          >
            <view class="my-decorate-service">
              <view class="service-title flex-space-between-row">
                ...
                <view
                  class="r flex-start-row"
                  v-if="aServiceData.myServiceFlag"
                  @click="goToMyDecorate"
                >
                  <text>查看全部</text>
                  <image src="https://ali-image.dabanjia.com/static/mp/dabanjia/images/decorate/ic_more.svg">
                  </image>
              ...
              <service-item
                v-for="(item, index) in purchasedServiceList"
                :key="item.nodeType"
                :serviceData="item"
              ...
              <!--  -->
              <view
                v-if="aServiceData.projectStatus == 3 && purchasedServiceList.length == 0"
                class="jun-gong-da-ji"
              >
                <image
                  mode="aspectFit"
                  src="https://ali-image.dabanjia.com/static/mp/dabanjia/images/decorate/img_finish.webp"
                ></image>
              </view>
              <no-service
                v-if="aServiceData.projectStatus != 3 && purchasedServiceList.length == 0"
          ...
          <view class="tips-design-actuary">
            <view
              v-if="availGuides.length > 0"
              class="tips"
            >
              购买相关服务 即刻开启装修
            </view>
            <guide-card
              v-if="availGuides.includes('design')"
              cardType="service"
            ...
            <guide-card
              v-if="availGuides.includes('actuary')"
              cardType="actuary"
          ...
          <uni-popup ref="sw">
            <house-switch
              ...
              :current="currentProject.uid"
              @goAddHouse="addHouse"
              @checkHouse="changeCurrentProject"
            ></house-switch>
          </uni-popup>
          <decorate-notice
            ...
            v-if="noticeActive"
            :num='msgNum'
            :current='currentProject.projectId'
            @closeNotice='closeNotice'
            class="decorate-notice"
          ></decorate-notice>
        </view>
        <drag-button-follow
          v-if="msgNum>0&&dragButton"
          :num='msgNum'
          :style.sync="style"
          @btnClick='openNotice'
          :follow='`left,right`'
  ...
  methods: {
    ...
    consultingService() {
      ...
      uni.showModal({
        title: "",
        content:
          "为了您房屋装修可以得到更好的保障，建议您购买工地保险，详情可咨询客服",
        confirmText: "去咨询",
        success: (res) => {
          if (res.confirm) {
            console.log("点击了确认");
    ...
    toFriends() {
      uni.navigateTo({
        url:
          "/sub-decorate/pages/friends/friends?id=" +
    ...
    getProjectList() {
      getEstateProjectInfoList({
        ...
        if (data && data.length > 0) {
          ...
          const { currentProject, switchFlag, msgProjectId } =
            getApp().globalData;
          // 设置当前的项目
          let arr = [];
    ...
    gonohousedecatore(type) {
      if (this.currentEstate && this.currentEstate.id) {
        ...
        if (this.currentProject && this.currentProject.projectId) {
          url = `/sub-decorate/pages/no-house-decorate/no-house-decorate?type=${type}&estateId=${this.currentProject.estateId}&projectId=${this.currentProject.projectId}&isDecorate=1&from=decorateIndex`;
        } else {
          url = `/sub-decorate/pages/no-house-decorate/no-house-decorate?type=${type}&estateId=${this.currentProject.estateId}&isDecorate=1&from=decorateIndex`;
        }
        uni.navigateTo({
          url,
        });
    ...
    goToMyDecorate() {
      uni.navigateTo({
        url:
          "/sub-decorate/pages/my-decorate/my-decorate?processId=" +
