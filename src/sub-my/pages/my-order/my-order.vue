<template>
	<view class="fill">
		...
		<view class="line" />
		<swiper class="swiper" :class="{empty:orderListLength<=0}" :current="currentIndex" :duration="200"
			@change="swiperChange">
						...
						<view class="order-container" v-for="item in orderList" :key="item.id">
							<view class="header">
								<view class="store-name" @click="gotoShop(item)">
									...
									<image v-if="!item.orderName || item.type == 5" src="../../static/ic_more.svg"
										mode=" " />
								</view>
							...
							<view class="body">
								...
								<view class="product-info"
									...
									<view class="product-img">
										<scroll-view scroll-x style="width: 100%; white-space: nowrap;">
											<image v-for="item2 in handleImage(item.details)" :key="item2"
									...
									<view class="total-price">
										<view class="product-price">
											...
											<text style="font-weight: 400;"
												class="price-font">{{handlePrice(item.totalAmount)[0]}}.</text>
											<text style="font-size:22rpx;"
												class="price-font">{{handlePrice(item.totalAmount)[1]}}</text>
										</view>
								...
								<view v-else v-for="item2 in item.details" :key="item2.id">
									<!-- 服务和材料 -->
									<order-item v-if="item2.type !== 5" :dataList="item2"
										@handleDetail="goToDetail(item)" />
									<!-- 储值卡 -->
									<store-calue-card-item v-if="item.orderStatus !==1 && item2.type==5">
							...
							<view class="price">
								<view class="total-pay">
									<text>
										...
										<text
											style="font-size: 22rpx;">{{handlePrice(item.orderTotalAmount)[0]}}.</text>
								...
								<view v-if="item.orderStatus == 0" class="need-pay">
									...
									<text style="color:#FF3347;margin-left: 8rpx;">
										...
										<text style="font-size: 32rpx;"
											class="price-font">{{handlePrice(item.orderReceivableAmount)[0]}}.</text>
										<text style="font-size:22rpx;"
											class="price-font">{{handlePrice(item.orderReceivableAmount)[1]}}</text>
									</text>
								</view>
								...
								<view v-else class="need-pay">
									...
									<text v-if="item.orderStatus == 3" style="color:#333333;margin-left: 8rpx;">
										<text style="font-size:18rpx;">￥</text>
										<text style="font-size: 32rpx;"
											class="price-font">{{handlePrice(item.payAmount)[0]}}.</text>
										<text style="font-size:18rpx;"
											class="price-font">{{handlePrice(item.payAmount)[1]}}</text>
									</text>
									<text v-else style="color:#333333;margin-left: 8rpx;">
										<text style="font-size:18rpx;">￥</text>
										<text style="font-size: 32rpx;"
											class="price-font">{{handlePrice(item.totalActualIncomeAmount)[0]}}.</text>
										<text style="font-size:18rpx;"
											class="price-font">{{handlePrice(item.totalActualIncomeAmount)[1]}}</text>
									</text>
								</view>
							</view>
							...
							<view class="footer"
								...
								<view v-if="item.showCancelOrderTime" class="set-interval">
									...
									<view class="time-text">
										...
										<count-down :start="item.remainTime" :timeBackground="'#E4E6E6'"
											:timeColor="'#333333'" :separatorColor="'#333333'" @finish="onRefresh(e)">
										</count-down>
								...
								<view v-if="item.showToPayBtn || item.showCancelBtn" class="waitPayBottom">
									<view v-if="item.showCancelBtn" class="cancel-order"
