<template>
  ...
  >
    ...
    >
      <image
        src="https://ali-image.dabanjia.com/static/mp/dabanjia/images/my/blank_ic%402x.png"
        class="noGoodsImg"
      ></image>
    ...
    >
      ...
      >
        ...
        <view class="shopInfo">
          <view style="width: 36rpx;height: 36rpx;">
            <view
              class="check"
              v-if="!shopItem.shopChecked"
              @click="checkShop(shopItem.storeId)"
            ></view>
            <image
              class="checked"
              v-else
          ...
          <view
            class="goShop"
            @click="toShopHome(shopItem.storeId)"
          >
        ...
        <view class="freeMail">
          <view
            ...
            v-if="isDefault"
            :key="index"
          >每满{{shopItem.freeShippingThreshold===0?'0.00'
          ...
          >
            <view class="goodsItem">
              <view style="width: 36rpx;height: 36rpx;">
                <view
                  class="check"
                  v-if="!goodsItem.goodsChecked"
                  @click="checkGoods(shopItem.storeId,goodsItem.skuId)"
                ></view>
                <image
                  class="checked"
                  v-else
              ...
              <image
                :src="goodsItem.image+'?x-oss-process=image/resize,m_mfit,w_96,h_96'"
              ...
              <view class="goodsInfo">
                <view>
                  ...
                  <view
                    class="goodsSpec"
                    @click="openSpec(goodsItem.skuId,goodsItem.goodsChecked)"
                  >
                ...
                <view class="foot">
                  ...
                  <view class="countCtrl">
                    ...
                    > {{goodsItem.buyCount}} </view>

                    <image
                      class="inc"
                      @click="changeCount(true, shopIndex,goodsIndex)"
      ...
      >
        <view class="popupClass">
          ...
          <view
            class="service"
            :class="{'activeBorder':serviceChecked}"
          >
            <view
              class="popupTop"
              :class="{'activeBg':serviceChecked}"
            >
              <view class="left">
                <view
                  class="popupTitle"
                  :class="{'activeColor':serviceChecked}"
                >服务商品</view>
                <view
                  class="popupCount"
                  :class="{'activeColor':serviceChecked}"
                >{{serviceList.length}}件</view>
              </view>
              <view
                class="check"
                v-if="!serviceChecked"
                @click="checkSame(true)"
              ></view>
              <image
                class="checked"
                v-else
          ...
          <view
            class="entity"
            :class="{'activeBorder':entityChecked}"
          >
            <view
              class="popupTop"
              :class="{'activeBg':entityChecked}"
            >
              <view class="left">
                <view
                  class="popupTitle"
                  :class="{'activeColor':entityChecked}"
                >实物商品</view>
                <view
                  class="popupCount"
                  :class="{'activeColor':entityChecked}"
                >{{entityList.length}}件</view>
              </view>
              <view
                class="check"
                v-if="!entityChecked"
                @click="checkSame(false)"
              ></view>
              <image
                class="checked"
                v-else
      ...
      <view
        class="disabledSku"
        v-if="disabledSkuList.length"
      >
        <view class="top">
          <view class="title">已失效商品</view>
          <view
            class="clear"
            @click="clearDisaledSku"
          >清空失效商品</view>
        </view>
        <view
          class="disabldSkuItem"
          @click="toGoodsDetail(disabldSkuItem.skuId,true)"
          v-for="disabldSkuItem in disabledSkuList"
          :key="disabldSkuItem.skuId"
        >
          <image
            :src="disabldSkuItem.image+'?x-oss-process=image/resize,m_mfit,w_96,h_96'"
            class="disabldSkuImg"
          ></image>
        ...
        if (storeList.length) {
          storeList.map((item) => {
            item.skuList.map((ele) => {
              ele.isMiniOrder =
                +ele.buyCount <= +ele.minimumOrderQuantity ? true : false;
