  ...
  >
    ...
    >
      ...
      <view class="state-bar">
        <view
          v-for="(item,index) in roomInfo.interactionInfo"
          :key="item.id"
        >

          <view
            v-if="index==0||item.connectStatus=='connecting'"
            class="user"
        ...
        <view class="state-bar-text">
          <image
            class="img"
            src="http://dbj.dragonn.top/static/mp/dabanjia/images/home/ic_gkrs.png"
            mode=""
          >
          </image>
      ...
      <view class="bottom-contain">
        ...
        >
          <view class="chat-item-height">
            ...
            >
              <view
                v-if="item.type=='TIMTextElem' || item.type=='TIMImageElem'"
                class="chat-item"
              >
                <view class="avater">
                  <image
                ...
                <image
                  class="anchor"
                  v-if="item.from.startsWith('anchor')"
                  src="http://dbj.dragonn.top/static/mp/dabanjia/images/home/anchor.png"
                ></image>
                ...
                <image
                  ...
                  mode="aspectFit"
                  @click="previewImg(item.payload.imageInfoArray[0].imageUrl)"
                > </image>
              </view>

              <view
                v-if="item.type=='TIMGroupTipElem'&&item.payload.operationType==joinType"
                class="chat-item"
              >
                <text class="name">{{item.nick}} </text>
                <text>进入直播间</text>
              ...
              <view
                ...
                class="chat-item"
                @click="toGoodsDetail(item.formatData.params)"
              >
                <view class="avater">
                  <image
                    class="img"
                ...
                <text class="name">{{item.nick}} </text>
                <image
                  v-if="item.formatData.type=='img_message'"
                  class="img"
                  :src="item.formatData.fileUrl"
                  mode="aspectFit"
                  @click="previewImg(item.formatData.fileUrl)"
                > </image>
                <view
                  v-if="item.formatData.type=='group_product'"
                  class="product"
                >
                  ...
                  <view class="product-info">
                    <image
                      ...
                      :src="item.formatData.params.imageUrl"
                      mode=""
                    >
                    </image>
    ...
    >
      <image
        class="img"
        src="http://dbj.dragonn.top/static/mp/dabanjia/images/home/un-liveing.png"
        mode=""
      >
      </image>
      <view class="tips">
...
import { publicRoom, insertAndGetLikeNum } from "../../../api/home.js";
...
export default {
  ...
  methods: {
    ...
    toPersonal({ index, item }) {
      if (index == 0) {
        uni.navigateTo({
          url:
            "../../../sub-decorate/pages/person-page/person-page?personId=" +
            item.userId,
        });
      }
    },
