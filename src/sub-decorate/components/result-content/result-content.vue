<template>
  <view class="result-content">
    <view
      class="report-time"
      v-show="!isReport&&checkData.time"
    >{{checkData.time}}后将会自动确认验房结果</view>
    <view
    ...
    >
      ...
      <text
        class="report-text"
        :class="{'report-text-hidden':isHidden}"
      >{{checkData.summaryDescription}}</text>
      <view
        class="openHidden"
        v-if="showBtn"
        @click="clickHidden"
    ...
    <view
      class="sticky"
      v-if="isActive&&!isReport"
    >
      <view
        class="item"
        :class="{'item-active':!isReport&&currentItem==='top'}"
        @click="toItem('top')"
      >
        重大隐患({{data[2].value}})</view>
      <view
        class="item"
        :class="{'item-active':!isReport&&currentItem==='hazardTop'}"
        @click="toItem('hazardTop')"
      >
        隐患({{data[1].value}})</view>
      <view
        class="item"
        :class="{'item-active':!isReport&&currentItem==='conformTop'}"
        @click="toItem('conformTop')"
      >
    ...
    >
      <deliverCard
        id="major-hazard"
        backgroundColor="linear-gradient(90.02deg, rgba(250, 77, 50, 0.09) 0.02%, rgba(250, 77, 50, 0.02) 99.99%);"
        color="#F5432D"
        title="重大隐患"
        :data='data[2]'
      ></deliverCard>
      <deliverCard
        id="hazard"
        backgroundColor="linear-gradient(90.02deg, rgba(254, 195, 101, 0.09) 0.02%, rgba(254, 195, 101, 0.02) 99.99%);"
        color="#FF7F1E"
        title="隐患"
        :data='data[1]'
      ></deliverCard>
      <deliverCard
        id="conform"
        backgroundColor="linear-gradient(90.02deg, rgba(74, 164, 252, 0.09) 0.02%, rgba(74, 164, 252, 0.02) 99.99%);"
        color="#4AA4FC"
        title="符合项"
        :data='data[0]'
      ></deliverCard>
...
export default {
  ...
  watch: {
    ...
    scrollTop() {
      if (this.scrollTop > this.top) {
        ...
        this.currentItem =
          this.scrollTop > this.conformTop
            ? "conformTop"
            : this.scrollTop > this.hazardTop
            ? "hazardTop"
            : "top";
      } else {
        this.isActive = false;
  ...
  methods: {
    getData() {
      ...
      getCheckResultDetail(this.serverId)
        .then((res) => {
          ...
          if (!this.isReport && this.checkData.autoSubmitTime > 0) {
            let id = setInterval(() => {
              ...
              this.checkData.autoSubmitTime =
                this.checkData.autoSubmitTime - 1000;
              this.checkData.time = this.toHHmmss(
                this.checkData.autoSubmitTime - 1000
              ).split(".")[0];
            }, 1000);
          }
