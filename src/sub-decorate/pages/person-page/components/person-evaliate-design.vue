<template>
  <view class="person-evaluate-design">
    <view
      class="peer-evaluate"
      v-if="peerComment.showCommentCount>0"
    >
      <image
        class="peer-bg"
        src="../../../static/person_peer.png"
        mode=""
      ></image>
      <view class="peer-title">
        <image
          :src="peerComment.avatar"
          @click="toPerson"
          mode=""
        ></image>
    ...
    >
      <image
        class="bg"
        src="../../../static/design_comment.png"
        mode=""
      ></image>
      <view
        class=""
        v-if="commentData.list[0]&&commentData.list[0].rank>3"
      >
        <view class="title">
          来自
        ...
        <view
          class="case-item"
          v-if="commentData.list[0].skuId!=0"
          @click="toBuy(commentData.list[0].skuInfoVO.skuId)"
        >
...
export default {
  ...
  methods: {
    toBuy(item) {
      ...
      uni.navigateTo({
        url:
          "/sub-classify/pages/goods-detail/goods-detail?goodId=" +
          item +
          "&userId=" +
          this.userId +
          "&originType=" +
          `2|${this.userId}|${this.userId}|`,
      });
    },
    toEvaluateList() {
    ...
    toPerson() {
      ...
      uni.navigateTo({
        url:
          ...
          this.peerComment.userId,
      });
    },
    toPeerList() {
      uni.navigateTo({
        url:
          "/sub-decorate/pages/person-page/person-peer-evaluate-list?id=" +
          this.peerComment.refUserId,
      });
    },
    // toRankList(){
    //   uni.navigateTo({
