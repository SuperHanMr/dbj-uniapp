<template>
  <view class="application-wrap">
    <view
      class="title-wrap add-item"
      v-if="changeOrderData.increasedItems"
    >
      <title
        :money="changeOrderData.increasedAmount"
        :label="msg.workTypeName+'服务增项费'"
        labelAmount="增项需补金额"
      ></title>
      <view class="list">
        <change-item
          v-for="(item, index) in changeOrderData.increasedItems"
          :itemData="item"
          :key="index"
        >
        </change-item>
    ...
    <view
      class="title-wrap subtract-item"
      v-if="changeOrderData.reducedItems"
    >
      <title
        :money="changeOrderData.reducedAmount"
        :label="msg.workTypeName+'服务减项费'"
        labelAmount="减项退还金额"
      ></title>
      <view class="list">
        <change-item
          v-for="(item, index) in changeOrderData.reducedItems"
          :itemData="item"
          :key="index"
        ></change-item>
      </view>
    ...
    <view class="summary">
      ...
      <view class="total-money">
        ...
        <view class="money price-font">{{changeOrderData.totalAmount < 0 ? "-" : ""}}￥<text class="ft-28">{{(Math.abs(changeOrderData.totalAmount)/100).toFixed(2)}}</text></view>
      </view>
    </view>
    <view
      class="store-money-card"
      v-if="changeOrderData.totalAmount > 0"
    >
      <view class="yu-e">
    ...
    <view class="pay-wrap">
      ...
      <view
        ...
        v-if="changeOrderData.totalAmount > 0"
        @click="submitCard"
      >同意并支付<text class="unit price-font">￥</text><text class="price-font">{{totalAmount}}</text></view>
      <view
        class="b-t-1 b-t-p"
        v-if="changeOrderData.totalAmount === 0"
        @click="submit(0)"
      ...
      <view
        ...
        v-if="changeOrderData.totalAmount < 0"
        @click="submit(-1)"
      >同意并退还您<text class="unit price-font">￥</text><text class="price-font">{{(Math.abs(changeOrderData.totalAmount)/100).toFixed(2)}}</text></view>
    </view>
    <uni-popup
      ref="payDialog"
      type="bottom"
