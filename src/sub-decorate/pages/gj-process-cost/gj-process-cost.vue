<template>
  <view class="process-cost">
    <view
      class="artificial"
      v-if="msg.payStatus == 2 || msg.obtainType != 2"
    >
      <view class="title">
        <view>人工费用{{levelLabel}}
        </view>
        <view
          v-if="msg.payStatus != 2 && levelList.length > 0"
          class="change-level"
          @click="openPopUp"
        >更换等级
        </view>
    ...
    <view
      class="material-cost"
      :style="{paddingBottom:containerBottom * 2 + 48 + 88 + 'rpx'}"
...
import { LEVEL } from "../../../utils/dict.js";
import { createOrder } from "../../../api/order-center.js";
import { sellList } from "../../../api/decorate.js";
import { batchChangeLevel } from "../../../api/level.js";
...
export default {
  ...
  onShow() {
    ...
    if (this.selectedMaterialData?.categoryId) {
      const { origin, categoryId } = this.selectedMaterialData;
  ...
  methods: {
    ...
    setCurrentEditMaterialChangData() {
      const { categoryId, item } = this.selectedMaterialData;
      let origin = {};
      // 缓存里的旧存储的原始的辅材列表数据
      ...
      this.$nextTick(() => {
        for (let i = 0; i < this.dataOrigin.material.categoryList.length; i++) {
          if (
            ...
            for (
              let j = 0;
              j < this.dataOrigin.material.categoryList[i].itemList.length;
              j++
            ) {
              if (
    ...
    selectWp(obj) {
      this.$nextTick(function () {
        console.log(obj);
        const { val, originalId, categoryId, item } = obj;
