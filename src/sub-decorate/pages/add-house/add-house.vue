        ...
        <view class="content">
          <view class="form-item">
            ...
            <input
              ...
              @focus='getFocus("contactName")'
              @blur='changeFocus("contactName")'
              placeholder="请输入业主姓名"
            />
            <view
              class="icon-clear"
              v-if="addData.contactName&&focus.contactName"
              @touchstart="clear('contactName')"
            >
              <uni-icons
                color="#c0c4cc"
                size="15"
          ...
          <view class="form-item">
            ...
            <input
              ...
              type="number"
              name="input"
              v-model="addData.contactPhone"
              placeholder="请输入手机号"
              @focus='getFocus("contactPhone")'
              @blur='changeFocus("contactPhone")'
            />
            <view
              class="icon-clear"
              v-if="addData.contactPhone&&focus.contactPhone"
              @touchstart="clear('contactPhone')"
            >
              <uni-icons
                color="#c0c4cc"
                size="15"
          ...
          <view class="form-item">
            ...
            <input
              class="uni-input"
              placeholder-class="placeholder"
              :class="{disabled:!!(roomId&&isEdit)}"
              :disabled="!!(roomId&&isEdit)"
              @click="chooseMap"
              disabled
              name="input"
              v-model="addData.area||addData.locationName"
              placeholder="请选择您房屋所在地区"
            />
            <i
              src=""
              class="icon-alert_notice_jump shopIcon"
            ></i>
          </view>
          <view class="form-item">
            ...
            <input
              class="uni-input"
              placeholder-class="placeholder"
              :class="{disabled:!hasPoint||(!!(roomId&&isEdit))}"
              :disabled="!hasPoint||(!!(roomId&&isEdit))"
              name="input"
              v-model="addData.housingEstate"
              placeholder="房屋所在小区"
              @focus='getFocus("housingEstate")'
              @blur='changeFocus("housingEstate")'
            />
            <view
              class="icon-clear"
              v-if="addData.housingEstate&&!isEdit&&focus.housingEstate"
              @touchstart="clear('housingEstate')"
            >
              <uni-icons
                color="#c0c4cc"
                size="15"
        ...
        <view class="content">
          <view class="form-item">
            ...
            <input
              ...
              @blur='changeFocus("address")'
              :class="{disabled:!!(roomId&&isEdit)}"
              placeholder="例:16号楼5层301"
            />
            <view
              class="icon-clear"
              v-if="addData.address&&!isEdit&&focus.address"
              @touchstart="clear('address')"
            >
              <uni-icons
                color="#c0c4cc"
                size="15"
          ...
          <view class="form-item special">
            ...
            <choose-btn
              :btnList='floorList'
              :currentBtn='addData.houseStructure'
              :disabled="!!(roomId&&isEdit)"
              @chooseBtn='chooseFloor'
            >
            </choose-btn>
          </view>
          <view class="form-item">
          ...
          <view class="form-item">
            ...
            <view
              v-if="addData.insideArea"
              :class="{disabled:!!(roomId&&isEdit)}"
              class="uni-input area-text"
            ><text style="visibility: hidden">{{addData.insideArea}}</text>m²</view>
            <view
              class="icon-clear"
              v-if="addData.insideArea&&!isEdit&&focus.insideArea"
              @touchstart="clear('insideArea')"
            >
              <uni-icons
                color="#c0c4cc"
                size="15"
            ...
            <input
              ...
              :maxlength="7"
              :class="{disabled:!!(roomId&&isEdit)}"
              class="uni-input house-area"
              :disabled="!!(roomId&&isEdit)"
              placeholder-class="placeholder"
              @focus='getFocus("insideArea")'
              @blur='changeFocus("insideArea")'
              placeholder="请输入房屋面积"
              type="digit"
              name="input"
              v-model="addData.insideArea"
            />
          ...
          <view class="form-item special ele">
            ...
            <choose-btn
              :btnList='elevatorList'
              :currentBtn='addData.hasLift'
              :disabled="!!(roomId&&isEdit)"
              @chooseBtn="chooseEle"
            >
            </choose-btn>
            <view class="floor-content">
              ...
              <input
                v-if="!addData.hasLift"
                :class="{disabled:!!(roomId&&isEdit)}"
                :disabled="!!(roomId&&isEdit)"
                placeholder-class="placeholder"
                class="ele-input"
                name="input"
                v-model="addData.floors"
                placeholder="请输入房屋所在楼层"
                type="number"
                :maxlength="3"
              />
              <view
                class="icon-clear-spec"
                v-if="!addData.hasLift&&addData.floors"
                @touchstart="clear('floors')"
              >
                <uni-icons
                  color="#c0c4cc"
                  size="15"
...
export default {
  ...
  methods: {
        ...
        success: function (res) {
          if (res.address) {
            ...
            uni.request({
              ...
              //web端ak:hAhAUtphDqdvj4QtHXXIGoC4RW8HeLc1
              //小程序端ak:lrRwp5WQK6fyjnyHGbyHBgFIXXczCIWN
              url:
                "https://api.map.baidu.com/reverse_geocoding/v3/?ak=lrRwp5WQK6fyjnyHGbyHBgFIXXczCIWN&output=json&coordtype=gcj02ll&location=" +
                res.latitude +
                "," +
                res.longitude,
              success: (res) => {
    ...
    async getAreaId(adcode) {
      let { areaId, cityId, provinceId } = await getAdcodeFromAreaId(adcode);
    ...
    addressChange(res) {
      ...
      this.$jsonp(
        "https://api.map.baidu.com/reverse_geocoding/v3/?ak=hAhAUtphDqdvj4QtHXXIGoC4RW8HeLc1&output=json&coordtype=gcj02ll&output=json&location=" +
          res.latitude +
          "," +
          res.longitude +
