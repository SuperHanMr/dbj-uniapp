<template>
	<view>
		<custom-navbar :opacity="sollTop/100" :title="hea	itle">
			<template v-slot:back>
				<i
					class="icon-ic_cancel_white back-icon"
					:style="{		 (scrollTop/		?'black':'white'}"
					@click="back"
				></i>
			</template>
		</custom-navbar>
		<view>
			<view class="basic-info" :style="{backgroundImage:`u			mg})`,backgroundSi				100%'}">
				<view :style="			nav		ght}"></view>
				<view class="info">
					<view class="header1">{{he	Title}}</view>
				</view>
				<view class="header2">{{de	.refundTime 		atDate}}</view>
				<view class="placehold"></view>
			</view>
			<view v-if="![0, 1]			s(detail.status)" class="header-content">
				<view v-if="tips" class="icon">
					<i class="icon-icon_order_tips"></i>
				</view>
				<view
					class="alert"
					:style="{color:detail.s				#FF3347':'#333333'}"
					style="flex:1"
				>{{tips}}</view>
			</view>
		</view>
		...
		<view class="info-content">
			...
			<view class="row">
				<view class="heaer">退款原因：</view>
				<view class="text">{{detail.re as on}}</view>
			</view>

			<view class="row">
				<view class="heaer">退款类型：</view>
				<view class="text">{{refuntType		l.t	}}</view>
			</view>

			<view class="row">
				<view class="heaer">退款金额：</view>
				<view class="text">￥{{detail.refundAmount}}</view>
			</view>

			<view class="row">
				<view class="heaer">申请时间：</view>
				<view class="text">{{detail.createTime | formatDate}}</view>
			</view>

			<view class="row">
				<view class="heaer">退款编号：</view>
				<view class="text">
					{{detail.refundNo}}
					<text class="tip" @click="copy(detail.refundNo)">复制</text>
				</view>
			</view>

			<view class="row">
				<view class="heaer">备注信息：</view>
				<view class="text">{{detail.remark}}</view>
			</view>
		</view>
		<view v-if="[4,5].includes(detail.status)&&ownered" class="other-btn" @click="toMessage">联系客服</view>
		<view v-if="[4,5].includes(detail.status)&&ownered" class="other-btn" @click="reApply">重新申请</view>
