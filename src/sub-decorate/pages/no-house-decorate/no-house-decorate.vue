		...
		<view class="content" v-if="noData === 1">
			<view class="addhouse-decs" v-if="!currentHouse.id">
				<button class="addhouse" @click="goAddHouse">
					<image
						src="https://ali-image.dabanjia.com/static/mp/dabanjia/images/decorate/ic_add_house_info.svg">
					</image>
					<text>添加房屋信息</text>
				</button>
			...
			<service-card v-if="(sssType == 'decorate' || sssType == 'design') && design.id" :setting="design"
				:showLevel="showLevel" class="service-card" @selectAnother="selectAnotherHandler('design')"
				@changeLevel="open">
				<template slot="check">
