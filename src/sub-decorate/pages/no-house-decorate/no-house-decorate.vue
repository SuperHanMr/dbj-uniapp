<template>
  <view
    class="no-house-decorate"
    :class="{center: noData === 0}"
  >
    <view
      class="content"
      v-if="noData === 1"
    >
      ...
      <my-current-house
        v-if="currentHouse && currentHouse.id"
        :houseData="currentHouse"
        @changCurrentHouse="changCurrentHouse"
      >
      </my-current-house>
      <service-card
        v-if="(sssType == 'decorate' || sssType == 'design') && design.id"
        :setting="design"
        :showLevel="showLevel"
        class="service-card"
        @selectAnother="selectAnotherHandler('design')"
        @changeLevel="open"
      >
        <template slot="check">
          <check-box
            :checked="design.checked"
            @change="(value)=> {change(design.cardtype, value)}"
          >
          </check-box>
        </template>
      </service-card>
      <service-card
        v-if="(sssType == 'decorate' || sssType == 'actuary') && actuary.id"
        :setting="actuary"
        class="service-card"
        @selectAnother="selectAnotherHandler('actuary')"
      >
        <template slot="check">
          <check-box
            :checked="actuary.checked"
            @change="(value)=> {change(actuary.cardtype, value)}"
          >
          </check-box>
        </template>
      </service-card>
      <service-card
        v-if="sssType == 'checkHouse' && checkHouse.id"
        :setting="checkHouse"
        class="service-card"
        @selectAnother="selectAnotherHandler('checkHouse')"
        @setDoorTime="openDoorTimePopup"
      >
        <template slot="check">
          <check-box
            :checked="checkHouse.checked"
            @change="(value)=> {change(checkHouse.cardtype, value)}"
          >
          </check-box>
        </template>
      </service-card>
...
import { queryEstates, getServiceSku } from "../../../api/decorate.js";
import { queryMaxLevel, changeLevel } from "../../../api/level.js";
...
export default {
  ...
  methods: {
    ...
    getServiceSku() {
      let params = {
        provinceId:
          this.currentHouse.provinceId || this.defaultHouse.provinceId,
        cityId: this.currentHouse.cityId || this.defaultHouse.cityId,
      ...
      getServiceSku(params).then((data) => {
        const { serviceType, values } = this.selectedServer;
        if (serviceType == 1) {
          this.design = {
            title: "设计服务",
            cardtype: "design",
    ...
    selectAnotherHandler(pp) {
      ...
      if (pp === "design") {
        str = `/sub-decorate/pages/service-list/service-list?name=设计服务&serviceType=1&areaId=${areaId}&insideArea=${this.currentHouse.insideArea}&spuId=${this.design.spuId}&categoryId=${this.design.categoryTypeId}&unitId=${this.design.unitId}`;
      }
      if (pp === "checkHouse") {
        str = `/sub-decorate/pages/service-list/service-list?name=验房服务&serviceType=2&areaId=${areaId}&insideArea=${this.currentHouse.insideArea}&spuId=${this.checkHouse.spuId}&categoryId=${this.checkHouse.categoryTypeId}&unitId=${this.checkHouse.unitId}`;
      }
      if (pp === "actuary") {
        str = `/sub-decorate/pages/service-list/service-list?name=精算服务&serviceType=4&areaId=${areaId}&insideArea=${this.currentHouse.insideArea}&spuId=${this.actuary.spuId}&categoryId=${this.actuary.categoryTypeId}&unitId=${this.actuary.unitId}`;
      }
      uni.navigateTo({
        url: str,
      });
    ...
    checkServiceCardIsSlected(type) {
      this.tips = {
        ...
        checked: true,
        content:
          "精算是将您的设计图内容转化为商品或服务消费的过程，是打扮家装修平台的特色服务；您也可以单独验房设计服务，之后再选择是否继续验房精算以及施工服务。",
      };
