<template>
	<view class="no-house-decorate">
		<view class="content">
			<view class="addhouse-decs" v-if="!currentHouse.id">
				<button class="addhouse" @click="goAddHouse">
					<image src="http://dbj.dragonn.top/static/mp/dabanjia/images/decorate/ic_add_house_info.svg">
					</image>
					<text>添加房屋信息</text>
				</button>
		...
		<uni-popup ref="level">
			<change-level @changeLevel="setLevel" @close="close" :dataList="levelList" :current="levelList[0]">
			</change-level>
		</uni-popup>
		<uni-popup ref="tips">
	...
	export default {
		...
		computed: {
			...
			countPrice() {
				...
				let temp =
					dprice * this.currentHouse.insideArea +
					aprice * this.currentHouse.insideArea +
					chprice * this.currentHouse.insideArea;
				return temp;
			},
		},
		onLoad(option) {
		...
		methods: {
			...
			getServiceSku() {
				let defaultHouse = getApp().globalData.currentHouse;
