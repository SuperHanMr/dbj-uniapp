    ...
    <view class="content">
      ...
      <view class="text-area" v-if="currentId === 4">
        <textarea
          ...
          class="replace-text-area"
          :maxlength="500"
          placeholder="请输入原因详情"
        />
        <text class="text-area-warning" v-if="!otherReason">（必填）</text>
        <text class="text-area-info" v-if="otherReason"
          >{{ otherReason.length > 500 ? 500 : otherReason.length }}/500</text
        >
      </view>
    </view>
...
<script>
import { rejectReson, refuseAreaChangeOrder } from "../../../api/decorate.js";
export default {
  data() {
    return {
      num: 0,
  ...
  onLoad(e) {
    (this.changeOrderId = e.changeOrderId || 1), this.rejectReson();
  },
  methods: {
    rejectReson() {
    ...
    submitQuery() {
      let data = {
        ...
        remark:
          this.currentId !== 4
            ? this.items[this.currentId - 1].reason
            : this.otherReason.substr(0, 500),
      };
