...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      uni.createSelectorQuery().select(".major-hazard").boundingClientRect((res)=>{
            this.top = res.top;
      }).exec()
      uni.createSelectorQuery().select(".hazard").boundingClientRect((res)=>{
            
            this.hazardTop = res.top;
      }).exec()
      uni.createSelectorQuery().select(".conform").boundingClientRect((res)=>{
            
            this.conformTop = res.top;
      }).exec()
    },
    onPageScroll(scrollTop){
      if(scrollTop.scrollTop>this.top){
        this.isActive = true
        this.currentItem = 'top'
        this.currentItem = scrollTop.scrollTop>this.conformTop?'conformTop':scrollTop.scrollTop>this.hazardTop?'hazardTop':'top'
